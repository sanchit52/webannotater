...
const highlightsToolsElement = document.getElementById('tools');
...
function hideTools() {
    highlightsToolsElement.style.display = 'none';
}
hideErrorState(); // Hide by default
hideTools();

function showTools() {
    highlightsToolsElement.style.display = 'flex';
}
...
function i18n() {
    ...
    const valueElements = document.querySelectorAll('[data-i18n-value-id]');
    valueElements.forEach((element) => {
      const key = element.getAttribute('data-i18n-value-id');
      const translation = chrome.i18n.getMessage(key);
      if (translation) {
